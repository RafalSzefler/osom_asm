...
use osom_asm_x86_64::{assembler::X86_64Assembler, models::{Condition, Immediate, Instruction, Label, GPR}};
use rstest::rstest;

#[test]
fn test_simple_mov() {
    let mut assembler = X86_64Assembler::new(false);
    assembler.emit(Instruction::Mov_RegImm { dst: GPR::RAX, src: Immediate::new(0) }).unwrap();
...
fn test_jmp(#[case] with_relaxation: bool, #[case] expected: &[u8]) {
    ...
    assembler.emit(Instruction::Xor_RegReg { dst: GPR::RAX, src: GPR::RAX }).unwrap();
...
fn test_jmp_cond(#[case] with_relaxation: bool, #[case] expected: &[u8]) {
    ...
    assembler.emit(Instruction::SetPrivate_Label { label }).unwrap();
    assembler.emit(Instruction::Xor_RegReg { dst: GPR::RAX, src: GPR::RAX }).unwrap();
    assembler.emit(Instruction::CondJump_Label { condition: Condition::Above, dst: label }).unwrap();
    assembler.emit(Instruction::Ret).unwrap();
